...
> **補足**: `fetch`リクエストは、すべての`generate`プレフィックス関数、レイアウト、ページ、サーバーコンポーネント間で同じデータに対して自動的に[メモ化](/docs/app/building-your-application/caching#request-memoization)されます。`fetch`が利用できない場合は、React[`cache`を使用](/docs/app/building-your-application/caching#react-cache-function)できます。
