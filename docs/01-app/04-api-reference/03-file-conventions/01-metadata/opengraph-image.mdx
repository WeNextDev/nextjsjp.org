...
> - `opengraph-image.js`と`twitter-image.js`は、[Dynamic API](/docs/app/building-your-application/caching#dynamic-apis)や[dynamic config](/docs/app/building-your-application/caching#segment-config-options)オプションを使用しない限り、デフォルトでキャッシュされる特殊なRoute Handlersです。
