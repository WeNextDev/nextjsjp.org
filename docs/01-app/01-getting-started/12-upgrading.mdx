...
Next.jsの最新バージョンにアップデートするには、`upgrade` コードモッドを使用できます：

```bash filename="Terminal"
npx @next/codemod@canary upgrade latest
```

手動でアップグレードしたい場合は、最新のNext.jsとReactバージョンをインストールしてください：
...
最新のcanaryにアップデートするには、Next.jsの最新バージョンを使用していて、すべてが期待通りに動作していることを確認してください。その後、次のコマンドを実行します：
